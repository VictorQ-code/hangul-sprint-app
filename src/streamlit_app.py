# src/streamlit_app.py (Versión final con auto-corrección de niveles)
...
# --- ¡NUEVA FUNCIÓN INTELIGENTE PARA EL MODO LECTURA! ---
def load_new_reading_item(attempt_count=0):
    """
    Carga un nuevo item de lectura. Si encuentra una frase demasiado larga,
    la mueve al nivel 5 y busca otra.
    """
    MAX_WORDS_FOR_READING = 10  # Define el máximo de palabras para este modo
    MAX_ATTEMPTS = 10 # Para evitar bucles infinitos

    if attempt_count >= MAX_ATTEMPTS:
        st.error("No se pudo encontrar una frase adecuada. Por favor, intenta con otro nivel.")
        st.session_state.practice_text_info = None
        return
    ...
    if item:
        # Comprobamos la longitud de la frase
        if len(item['hangul'].split()) > MAX_WORDS_FOR_READING:
            st.info(f"Frase demasiado larga encontrada... Moviéndola al nivel 5 y buscando otra.")
            # Movemos la frase al nivel 5 en la base de datos
            stats.update_phrase_level(item['id'], 5)
            # Volvemos a llamar a la función para buscar una nueva frase
            load_new_reading_item(attempt_count + 1)
        else:
            # La frase tiene una longitud adecuada, la cargamos
...
def setup_romanization_game():
    ...
    while items_fetched < 10 and attempts < 30:
        item = stats.get_short_practice_item(level=level, max_words=4) # Usa frases de máximo 4 palabras
        attempts += 1
        if item:
...
if st.session_state.page == "Leer":
    if not st.session_state.practice_in_progress:
        ...
        # --- ¡CAMBIO CLAVE! El slider ahora solo va de 1 a 4 ---
        st.session_state.reading_level = st.slider("Nivel de Dificultad (1-4 para frases cortas)", 1, 4, st.session_state.reading_level)
        
        st.divider()

        if st.button("🚀 Empezar a Leer", use_container_width=True, type="primary"):
            st.session_state.session_history = []
            with st.spinner("Cargando texto adecuado..."):
        ...
        if st.session_state.practice_text_info:
            ...
            if not st.session_state.timer_running and st.session_state.elapsed_time == 0:
                st.session_state.start_time, st.session_state.timer_running = time.time(), True
                st.rerun()

            if st.session_state.timer_running:
                st.info(" Cronómetro en marcha... ¡Haz clic en una palabra si necesitas ayuda!")
                words = korean_text.split()
                if len(words) > 10: # Doble seguridad, aunque la nueva función debería evitar esto
            ...
            if st.session_state.timer_running:
                ...
                if col2.button("Cargar Otro Texto", use_container_width=True):
                    with st.spinner("Buscando otro texto adecuado..."):
                        load_new_reading_item()
                    if st.session_state.practice_text_info:
                        if st.session_state.practice_text_info not in st.session_state.session_history:
                            st.session_state.session_history.append(st.session_state.practice_text_info)
                    st.rerun()
...
# ==============================================================================
    ...
    if not st.session_state.romanization_practice_in_progress:
        ...
        if st.session_state.current_question_index >= len(st.session_state.romanization_questions):
            ...
            correct_answers = 0; wrong_answers_details = []
            ...
            total_q = len(st.session_state.romanization_questions); current_idx = st.session_state.current_question_index
            current_q = st.session_state.romanization_questions[current_idx]
